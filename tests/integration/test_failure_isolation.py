"""US3 통합 — 소스 실패 격리(FR-009) + 일자 간 중복 제거(FR-011) + 과반 임계 게이트."""
...
def test_exit_code_partial_vs_total_failure(monkeypatch, tmp_path):
    monkeypatch.setattr(collect_mod, "_dispatch", _make_dispatch("hackernews"))
    _stub_enrich(monkeypatch)
    args = ["--date", TODAY, "--out", str(tmp_path), "--state", str(tmp_path)]
    ...
    monkeypatch.setattr(collect_mod, "_dispatch", _all_fail)
    assert collect_mod.main(args) == 1
...
def test_cross_day_dedup(monkeypatch, tmp_path):
    ...
    _stub_enrich(monkeypatch)
    # 1일차: 모든 항목 신규
    snap1 = collect_mod.collect(TODAY, tmp_path, state_dir=tmp_path)
    ...
    snap2 = collect_mod.collect(TOMORROW, tmp_path, state_dir=tmp_path)
